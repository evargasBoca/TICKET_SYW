...
def _payload(ticket_client, **overrides):
    payload = {
...
def test_create_accepts_accents_numbers_and_punctuation(client, ticket_client):
    response = client.post("/api/tickets", json=_payload(
        ticket_client, title="Error en informe (v2) — cliente Ñuñoa, línea 42"))
    assert response.status_code == 201, response.get_json()


def test_create_trims_surrounding_whitespace(client, ticket_client):
    response = client.post("/api/tickets", json=_payload(ticket_client, title="  Título con espacios  "))
    assert response.status_code == 201, response.get_json()
