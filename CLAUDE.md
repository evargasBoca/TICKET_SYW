...
**Active feature**: Cierre de OBS-0044–OBS-0047 (Backlog UAT ITER-008, reportadas por Arely Pazmiño) — corrige las 4 observaciones en estado `Abierta` de `UAT/02_Backlog/BACKLOG.md`: (1) OBS-0044, hora del historial de registros de tiempo desfasada ~5h respecto a la ingresada — causa raíz confirmada: `TimeLogModal.tsx`/`WorkSessionForm.tsx` hacían `iso.slice(11,16)` sobre el ISO-8601 en UTC devuelto por la API en vez de formatear a hora local (el dato en `work_sessions.started_at/ended_at` ya se guardaba correcto); ahora usan `date-fns` (`format(parseISO(...), 'HH:mm')`); (2) OBS-0045, la etiqueta "Fuera de jornada" se superponía al horario en la columna "Fecha" del historial — reubicada a la columna "Horario", debajo del texto del horario; (3) OBS-0046, no se notificaba al reasignar un ticket (`POST /api/tickets/{id}/reassign` nunca invocaba `NotificationService`, a diferencia de `/assign` que sí lo hace) — se agregó `event_type` `"reassigned"` y mensaje enriquecido (cliente, prioridad, estado, quién asignó) en ambos eventos; (4) OBS-0047, se podía asignar/reasignar un ticket a un resolutor con la cuenta de usuario desactivada — `AssignmentService`/`ReassignmentService` ya validaban `Resource.active` pero no el `User.active` vinculado (`Resource.user_id`), dos toggles independientes en `TeamPage.tsx` ("Desactivar recurso" vs. "Desactivar cuenta"); se agregó `Resource.user_active` (poblado vía `relationship` `viewonly` a `UserModel`) y el chequeo en ambos servicios + filtro en `useResourceCandidates.ts` + campo nuevo en `GET /api/resources`. Sin dependencias nuevas, sin migración de base de datos. Implementada — tasks 22/22, validada contra Docker real (OBS-0044/0045 verificadas registrando tiempo real en TK-000001 vía navegador logueado: historial y formulario de edición muestran la hora exacta ingresada sin desfase, etiqueta "Fuera de jornada" ya no se superpone; OBS-0047 verificada end-to-end vía `GET /api/resources` desde el navegador, confirmando `user_active: false` para un recurso con cuenta desactivada); `pytest tests/domain/test_notification_service.py tests/domain/test_assignment_service.py tests/domain/test_reassignment_service.py tests/api/test_reassign.py` en 20/20; `tsc -b` sin errores; `OBS-0044`–`OBS-0047` actualizadas a `Lista para Validar` en el backlog. Pendiente: commit y retest formal por el consultor UAT (dueña de las observaciones: Arely Pazmiño).
**Spec**: specs/032-cierre-obs-0044-0047/spec.md
**Plan**: specs/032-cierre-obs-0044-0047/plan.md
**Tasks**: specs/032-cierre-obs-0044-0047/tasks.md
**Research**: specs/032-cierre-obs-0044-0047/research.md
**Data model**: specs/032-cierre-obs-0044-0047/data-model.md
**Quickstart**: specs/032-cierre-obs-0044-0047/quickstart.md
