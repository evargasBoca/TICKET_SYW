# Research: Cierre de OBS-0060–OBS-0063 (ITER-009)
...
## Decisión 1 — OBS-0060: causa raíz confirmada en el formateo de hora en frontend, no en el almacenamiento
...
- **Lectura/presentación**: la API devuelve `ws.started_at.isoformat()` (`backend/api/routes/work_sessions.py:157-158`), que al venir de una columna `timezone=True` se normaliza a UTC (ej. `...T22:37:00+00:00` para un `17:37` local en Bogotá). El frontend, en dos lugares, hace `iso.slice(11, 16)` sobre ese string — una extracción de substring que asume (incorrectamente) que ya está en hora local, cuando en realidad toma la hora en UTC:
  - `frontend/src/components/worksessions/TimeLogModal.tsx:38-43` (`formatTimeRange`, historial de registros — el bug reportado en OBS-0060/evidencia).
...
## Decisión 2 — OBS-0061: reubicación visual del badge, mismo dato ya calculado
...
**Alternatives considered**: ícono con tooltip en vez de badge de texto — es la alternativa que menos espacio ocupa, mencionada en `ITER-009.md` como opción; se prefiere el badge en la columna "Horario" (más consistente con el patrón `Tag` ya usado en el resto de la tabla, ej. columna "Estado" de tickets) sobre introducir un ícono nuevo sin necesidad.

## Decisión 3 — OBS-0062: falta el disparo de notificación específicamente en `/reassign`, no en `/assign`
...
## Decisión 4 — OBS-0063: el resolutor asignable se valida contra `Resource.active`, pero no contra el `User.active` (cuenta de acceso) vinculado
...
- Bloquear solo en frontend (ocultar del selector) sin tocar el backend: rechazado — el mismo `ITER-009.md`/FR-011 exige que la API rechace también un intento directo que omita el selector, consistente con el patrón ya usado por el propio chequeo existente de `Resource.active` (defensa en profundidad, Principio IV de la Constitución).
