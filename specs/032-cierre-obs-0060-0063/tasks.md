...
# Tasks: Cierre de OBS-0060–OBS-0063 (Backlog UAT ITER-009)
...
**Tests**: Se incluyen tareas de test de dominio/API backend (pytest) para OBS-0062 y OBS-0063, ultra-limitadas (Constitución Principio VII — extienden archivos ya existentes, sin insertar más de 5-10 registros de prueba). OBS-0060/OBS-0061 son cambios puramente de presentación en frontend; no hay framework de test de UI configurado en este repo, por lo que su verificación es manual contra Docker vía `quickstart.md`.
...
- **[Story]**: A qué User Story pertenece (US1=OBS-0060, US2=OBS-0061, US3=OBS-0062, US4=OBS-0063)
...
Ninguno: las 4 observaciones tocan archivos distintos y no comparten lógica entre sí (OBS-0060/0045 son frontend de `worksessions/`; OBS-0062/0047 son backend de `tickets`/`assign`/`reassign`, con `useResourceCandidates.ts` como único punto de frontend compartido por OBS-0063 pero no por las otras tres). Sin tareas bloqueantes en esta fase.

**Checkpoint**: Las 4 historias pueden empezar de inmediato, en paralelo si hay más de un desarrollador.

---

## Phase 3: User Story 1 - Hora correcta en el historial de registros de tiempo (OBS-0060, Priority: P1) 🎯 MVP
...
**Independent Test**: Registrar tiempo manualmente indicando una hora conocida (ej. 17:37–17:38) y verificar que el historial y el formulario de edición muestran esa misma hora (ver Fase "OBS-0060" de `quickstart.md`).
...
- [X] T004 [US1] Verificación manual en Docker real: registrar tiempo con una hora conocida, confirmar en el historial (`TimeLogModal`) y en el formulario de edición que la hora coincide exactamente, sin desfase (depende de T002, T003) — ver Fase "OBS-0060" de `quickstart.md` — **verificado en Docker real** (TK-000001, admin): se registró tiempo 17:37–17:38; el historial mostró exactamente "17:37 – 17:38" (antes del fix habría mostrado ~22:37–22:38); el formulario de edición precargó `input[type=time]` con valores `17:37`/`17:38` (`document.querySelectorAll('input[type=time]')`); registro de prueba eliminado tras verificar

**Checkpoint**: User Story 1 funcional y verificable de forma independiente (SC-001).

---

## Phase 4: User Story 2 - La etiqueta "Fuera de jornada" no oculta el horario (OBS-0061, Priority: P2)

**Goal**: La etiqueta "Fuera de jornada" se muestra sin superponerse a la hora de inicio/fin del registro.

**Independent Test**: Generar un registro fuera de horario laboral y verificar que tanto el horario como la etiqueta son legibles simultáneamente (ver Fase "OBS-0061" de `quickstart.md`).
...
**Independent Test**: Asignar un ticket y luego reasignarlo a otro resolutor; confirmar que ambos reciben notificación con la información mínima y que lleva al detalle del ticket (ver Fase "OBS-0062" de `quickstart.md`).
...
**Independent Test**: Desactivar la cuenta (no el recurso) de un resolutor y confirmar que ya no es asignable ni seleccionable, sin afectar asignaciones previas (ver Fase "OBS-0063" de `quickstart.md`).
...
- [X] T022 Actualizar `UAT/02_Backlog/BACKLOG.md`: cambiar el `Estado` de `OBS-0060`, `OBS-0061`, `OBS-0062` y `OBS-0063` de `Abierta` a `Lista para Validar`, siguiendo `UAT/CONVENTIONS.md` (FR-015) — confirmar que `UAT/01_Iterations/ITER-009/ITER-009.md` no se edita en su contenido narrativo (depende de T004, T006, T013, T020, T021) — hecho; `ITER-009.md` no tocado (verificado con `git diff`)
...
- **User Story 1 (Phase 3, OBS-0060)**: Depende solo de Setup. Independiente de las otras 3.
- **User Story 2 (Phase 4, OBS-0061)**: Depende solo de Setup. Comparte archivo (`TimeLogModal.tsx`) con US1 — ver nota de paralelismo abajo.
- **User Story 3 (Phase 5, OBS-0062)**: Depende solo de Setup. Independiente de US1/US2/US4 en archivos, aunque toca el mismo endpoint (`tickets.py`) que US4.
- **User Story 4 (Phase 6, OBS-0063)**: Depende solo de Setup. Comparte `tickets.py`/servicios de asignación con US3 (archivos distintos dentro del mismo módulo — ver nota abajo).
- **Polish (Phase 7)**: Depende de que las 4 historias estén verificadas (T004, T006, T013, T020) y de los tests acotados (T021).

### User Story Dependencies

- **US1 (OBS-0060, P1)**: Sin dependencias de otras historias.
- **US2 (OBS-0061, P2)**: Sin dependencias funcionales de US1, pero **T005 toca el mismo archivo que T002** (`TimeLogModal.tsx`) — no ejecutar T002 y T005 en paralelo; secuenciar (T002 antes de T005, o viceversa) dentro de ese archivo.
- **US3 (OBS-0062, P1)**: Sin dependencias de US1/US2/US4 a nivel de requisitos, aunque T011/T012 y T018 (de US4) tocan el mismo archivo `tickets.py` — secuenciar entre sí si se implementan en paralelo por distintos desarrolladores.
- **US4 (OBS-0063, P1)**: Sin dependencias de US1/US2/US3 a nivel de requisitos; comparte `tickets.py` con US3 (ver nota anterior) y `assignment_service.py`/`reassignment_service.py` en archivos ya tocados por lógica de US3 solo indirectamente (T011 no toca `validate()`).
...
2. Completar Phase 3: User Story 1 (OBS-0060) — el defecto de mayor impacto en integridad de datos/SLA.
3. **Detener y validar**: probar User Story 1 de forma independiente contra Docker (Fase "OBS-0060" de `quickstart.md`).
4. Continuar con User Stories 2, 3 y 4, y Polish, para cerrar las 4 observaciones de ITER-009.

### Incremental Delivery

1. Setup → entorno y datos de prueba listos.
2. User Story 1 (OBS-0060) → validar independientemente.
3. User Story 2 (OBS-0061) → validar independientemente.
4. User Story 3 (OBS-0062) → validar independientemente.
5. User Story 4 (OBS-0063) → validar independientemente.
...
- No editar retroactivamente `UAT/01_Iterations/ITER-009/ITER-009.md` — todo cambio de estado va en `UAT/02_Backlog/BACKLOG.md` (T022).
