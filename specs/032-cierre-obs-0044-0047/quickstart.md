# Quickstart: Validación de OBS-0044–OBS-0047
...
- Entorno Docker de desarrollo levantado (`docker compose up`, ver `README.md` del repo), en una zona horaria distinta a UTC (ej. `America/Bogota`, ya usada por el seed de Aris) para poder reproducir/verificar el desfase de OBS-0044.
- Un ticket de prueba abierto, asignado a un recurso con horario laboral configurado.
- Dos recursos con rol Resolutor disponibles para asignar/reasignar (para OBS-0046/OBS-0047), uno de ellos con posibilidad de desactivar su cuenta de usuario sin afectar el resto del entorno de prueba.
- Acceso con un usuario con permiso `tickets:assign` (Coordinador).

## OBS-0044 — Hora del historial de registros de tiempo
...
## OBS-0045 — Etiqueta "Fuera de jornada"
...
## OBS-0046 — Notificación al asignar/reasignar
...
## OBS-0047 — Bloqueo de asignación a usuarios inactivos
...
1. Tras validar los cuatro puntos, actualizar `UAT/02_Backlog/BACKLOG.md`: `OBS-0044` a `OBS-0047` pasan de `Abierta` a `Lista para Validar` (FR-015).
2. Confirmar que `UAT/01_Iterations/ITER-008/ITER-008.md` no fue editado en su contenido narrativo.
