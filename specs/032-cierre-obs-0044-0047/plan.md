# Implementation Plan: Cierre de OBS-0044–OBS-0047 (Backlog UAT ITER-008)
...
**Constraints**: Ninguna corrección puede alterar el contrato de API existente de forma rompiente (ver `contracts/api-changes.md`); OBS-0047 no puede afectar asignaciones ya existentes (FR-014), solo nuevas.
...
- **Principio II (Clean Architecture, 3 capas)**: Sin violaciones. La corrección de OBS-0044/0045 vive enteramente en Capa 3 frontend (presentación). La de OBS-0046/0047 vive en Capa 1 (`AssignmentService`, `ReassignmentService`, `NotificationService`, dominio puro) y Capa 3 (`tickets.py`, orquestación). Ningún cambio introduce lógica de negocio en componentes React ni en rutas Flask directamente.
...
- **Principio IV (Seguridad en profundidad)**: OBS-0047 refuerza este principio (defensa en profundidad: validación tanto en el selector del frontend como en el backend, evitando bypass directo a la API).
- **Principio V (Gobernanza de librerías)**: Sin dependencias nuevas. `date-fns` (ya aprobado) se usa para el fix de OBS-0044.
...
├── domain/
│   ├── entities/
│   │   └── notification.py           # OBS-0046: agrega event_type "reassigned"
│   └── services/
│       ├── assignment_service.py     # OBS-0047: valida también User.active vinculado
│       ├── reassignment_service.py   # OBS-0047: idem + dispara notificación (OBS-0046)
│       └── notification_service.py   # OBS-0046: nueva plantilla + mensaje enriquecido
├── infra/
│   └── repositories/
│       └── resource_repo.py          # OBS-0047: expone/filtra por User.active vinculado (si aplica)
├── api/
│   └── routes/
│       └── tickets.py                # OBS-0046/0047: TicketReassign — notificación + validación
└── tests/
...
│   ├── components/
│   │   ├── worksessions/
│   │   │   ├── TimeLogModal.tsx      # OBS-0044 (formatTimeRange) + OBS-0045 (layout etiqueta)
│   │   │   └── WorkSessionForm.tsx   # OBS-0044 (timeOf, precarga de edición)
│   │   └── tickets/
│   │       └── useResourceCandidates.ts  # OBS-0047: excluir cuenta de usuario inactiva
│   └── services/
│       └── resourceService.ts        # OBS-0047: si el filtro se resuelve server-side
```
